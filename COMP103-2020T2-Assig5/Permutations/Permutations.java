...
public class Permutations {
     ...
     *
     * If you run out of memory, then only add the first 10000 permutations to the answer.
     */
    public void extendPermutation(Set<String> remainingItems, Stack<String> permutationSoFar, List<List<String>> allPermutations){
        if(remainingItems.isEmpty())
        {
            //++permutationCount;
            counter++;
            ArrayList<String> copy = new ArrayList<>(permutationSoFar);
            allPermutations.add(copy);
        }
        else
        {
            for(String item : remainingItems)
            {
                Set<String> r = new HashSet<>(remainingItems);
                r.remove(item);
                permutationSoFar.add(item);
                extendPermutation(r, permutationSoFar, allPermutations);
    ...
    /**
     * Setup GUI
     * Buttons to reset and run permutations, and pause display
