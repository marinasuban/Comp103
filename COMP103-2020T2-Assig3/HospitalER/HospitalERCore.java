...
public class HospitalERCore{
    ...
    public void run(){
        ...
        while (running){         // each time step, check whether the simulation should pause.
            ...
            // check for patients who finished treatments
            ArrayList<Patient> tempForDischarge = new ArrayList<>();
            for (Patient p : treatmentRoom) {
                // if treated then add to list of discharge
                if (p.completedCurrentTreatment()) {
                    tempForDischarge.add(p);
                }
            }
            // discharge treated patients
            for (Patient p : tempForDischarge) {
