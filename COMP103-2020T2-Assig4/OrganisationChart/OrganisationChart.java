...
public class OrganisationChart {
    ...
    public void setupGUI(){
        ...
        UI.addButton("Load", ()->{load(UIFileChooser.open("File with a Decision Tree"));});
    ...
    public void load(String filename){
        ...
        try{
            organisation = loadSubTree(new ArrayList<String>(Files.readAllLines(Path.of(filename))));
            redraw();
        }
    ...
    private Position foundByName = null;
    public void findPositionByName (Position pos, String nameToFind,boolean foundName) {
        if (foundName) return;
        if (pos.getRole().equals(nameToFind)) {
            foundByName = pos;
            foundName = true;
        }

        for (Position p : pos.getTeam()) {
            findPositionByName(p,nameToFind, foundName);
        }
    }

    public Position loadSubTree(List<String> lines){

        Position top = null;
        for(String line:lines){
            Scanner sc = new Scanner(line);
            String parentName = sc.next(); 
            Position parent = new Position(parentName);
            if (top!=null) { //if there is a ceo
                //search through team to find matching node to scan
                foundByName  = null;
                findPositionByName(top, parentName, false);

                if (foundByName != null) {
                    parent = foundByName; // point to exist team member and assign as current parent
                }
            }

            String childrenRaw = sc.nextLine().trim();
            String childrenRaw1 = childrenRaw.replace("[","");
            String childrenRaw2 = childrenRaw1.replace("]","");
            List<String> childrenFinal= new ArrayList<String>(Arrays.asList(childrenRaw2.split(",")));
            //UI.println("currently processing: "+ parent.toStringFull());
            //UI.println("children:" + childrenFinal.toString());
            for(String childInfo : childrenFinal){
                Scanner scan = new Scanner(childInfo);
                String childName = scan.next();
                double offSet = scan.nextDouble();
                Position child = new Position(childName,offSet);
                parent.addToTeam(child);
    ...
    public void save(){
        ...
        for(Position membersInfo : organisation.getTeam()){
            teamInfo.add(membersInfo.toStringFull());
        }
        try {
            PrintStream ps = new PrintStream(new File(fname));
            ps.println(organisation.getRole() + " "+ teamInfo );
            this.saveSubTree(organisation,ps);
    ...
    public void saveSubTree(Position p, PrintStream ps) {
        for(Position teamMember: p.getTeam()){
            if (teamMember.isManager()) {
                List<String> teamInfo1 = new ArrayList<String>();
                for(Position membersInfo : teamMember.getTeam()){
                    teamInfo1.add(membersInfo.toStringFull());
                }
                ps.println(teamMember.getRole() + " "+ teamInfo1 );
                saveSubTree(teamMember,ps);
            }
        }
    }
    ...
    private Position findPosition(double x, double y, Position pos){
        ...
        for(Position teamMember: pos.getTeam()){
            Position answer = findPosition(x,y,teamMember);
    ...
    private void movePosition(Position pos, Position target) {
        ...
        /*# YOUR CODE HERE */
        if(pos!=organisation && inSubtree(pos,target)==false){
     ...
     * The position must be vacant and cannot be a manager of another position.
    ...
    private boolean inSubtree(Position pos, Position subtree) {
        ...
        if(subtree==pos){return true;} // if a child of pos is target, return true
        else{
            for(Position position: pos.getTeam()){ //get child of pos
                boolean answer = inSubtree(position,subtree);
