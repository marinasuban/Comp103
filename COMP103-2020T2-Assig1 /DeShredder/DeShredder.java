...
public class DeShredder {
    ...
    private List<Shred> toStrip; // the List of shreds to move to (possibly null)
    private int toPosition;
    
    private int fromStripRow;
    private int toStripRow;
    ...
    public void setupGUI() {
        ...
        UI.addButton("Save strip as image", this::save);
    ...
    private void fillColors(Color[][] imgToSave, int row, int col, Shred shredToProcess) {
        int size = (int) shredToProcess.SIZE;
        Color[][] shredColors = shredToProcess.getColors();
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                imgToSave[(row*size)+i][(col*size)+j] = shredColors[i][j];
            }
        }
    }
    ...
    public void save(){
        int shredSize = 40; 
        int maxColumn = -1;
        int rows = this.completedStrips.size();
        //setup loop for setting dimensios of exported Image
        for (int row = 0; row < rows; row++) {
            int cols = completedStrips.get(row).size();
            if (cols > maxColumn) {maxColumn = cols;}
        }
        

        if (rows > 0 && maxColumn > 0) {
            Color[][] imageToSave = new Color[rows*shredSize][maxColumn*shredSize];  //image size
            for (int i = 0; i < rows*shredSize; i++) {
                for (int j = 0; j < maxColumn*shredSize;j++) {
                    if (imageToSave[i][j] == null) {
                        imageToSave[i][j] = new Color(255,255,255);
                    }
                }
            }
        
            for (int row = 0; row < rows; row++) {
                for (int col = 0; col < this.completedStrips.get(row).size(); col++) {
                //for each image in completed strips
                Shred shredToProcess = this.completedStrips.get(row).get(col);
                this.fillColors(imageToSave, row, col, shredToProcess);
                
                }
            }
            this.saveImage(imageToSave, "SavedImageOutput.png");
        }
    ...
    public void load(Path dir, int count) {
        ...
        for (int i=1; i<count+1; i++){
            this.allShreds.add(new Shred(dir,i));
        }
    }
    ...
    public void rotateList(){
        ...
        this.allShreds.add(0,this.allShreds.get(this.allShreds.size()-1));
        this.allShreds.remove(this.allShreds.size()-1);
        display();
    }
    ...
    public void shuffleList(){
        for (int i=0; i<allShreds.size(); i++){
            int num=(int)(Math.random()*(allShreds.size()-1));
            Shred store=this.allShreds.get(num);
            this.allShreds.set(num,this.allShreds.get(i));
            this.allShreds.set(i,store);
        }
        display();
    }
    ...
    public void completeStrip(){
        if (this.workingStrip.size() > 0)
        {
            this.completedStrips.add(this.workingStrip);
            this.workingStrip = new ArrayList<>();
            display();
        }
    }
    ...
    public void doMouse(String action, double x, double y){
        ...
        if (action.equals("released")){
            toStripRow  = getStripRow(y);
            toStrip = getStrip(y); // the List of shreds to move to (possibly null)
            toPosition = getColumn(x);     // the index to move the shred to (may be off the end)
            // perform the correct action, depending on the from/to strips/positions
            if (fromStrip != null && toStrip != null && fromStripRow < 2 && toStripRow < 2) {
                //moving between Original strip and working strip
                this.moveShred(fromStrip, fromPosition, toStrip, toPosition);
            }
            else if (fromStrip != null && toStrip != null && fromStripRow >=2 && toStripRow >= 2) {
                //organizing completed strips
                this.moveStrip(fromStrip, toStrip, fromStripRow-2, toStripRow-2);
            }
            else if (fromStrip != null && fromStripRow >=2 && toStripRow == 1) {
                //moving completedStrip back into working strip
                if (this.toStrip == null || this.toStrip.isEmpty()) {
                    this.completedStrips.remove(fromStrip);
                    this.workingStrip = fromStrip;
                }
                //todo check working strip is empty
            }
    ...
    public int getStripRow(double y) {
       return (int) ((y-TOP_ALL)/(SIZE+GAP));
    }
    
    public void moveStrip(List fromStrip, List toStrip, int fromStripRow, int toStripRow) {
        
        if ((fromStripRow != toStripRow) && (fromStrip != toStrip)) {
            this.completedStrips.remove(fromStrip);
            this.completedStrips.add(toStripRow,fromStrip);
        }
    }
    
    
    public void moveShred(List fromStrip, int fromPosition, List toStrip, int toPosition){
        Shred store = this.fromStrip.get(fromPosition);
        this.fromStrip.remove(fromPosition);
        if (fromStrip == toStrip) { 
            this.toStrip.add(toPosition,store); 
        }  
        else {
            this.toStrip.add(store);
        }
    }
