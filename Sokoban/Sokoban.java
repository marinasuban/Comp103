...
public class Sokoban {
    ...
    Stack<ActionRecord> actionH = new Stack<>();
    Stack<ActionRecord> undoH = new Stack<>();
    ...
    public void moveOrPush(String direction) {
        ...
        if (action != null)
            this.actionH.push(new ActionRecord(action, direction));
    }
    ...
    public void doMouse(String action, double x, double y){
        ...
        int cellPressedColumn = (int) ((x-LEFT_MARGIN) / CELL_SIZE) ; //this is the column index of the cell that was clicked
        int cellPressedRow = (int) ((y-TOP_MARGIN) / CELL_SIZE);
        int distance = 0;
        //LEFT_MARGIN 
        //TOP_MAGRGIN
        //CELL SIZE    
        ...
        if (action.equals("released")){
            ...
            if (cellPressedColumn == this.workerPos.col) {
                // limits to straight paths only
                if (cellPressedRow > this.workerPos.row) { //going down
                    for ( int i = this.workerPos.row; i <= cellPressedRow;i++) {
                        if (!this.cells[i][this.workerPos.col].isFree() || this.cells[i][this.workerPos.col].hasBox()) {
                            isValidRoute = false;

                        }
                        else{
                            workerDir = "down";
                            distance=cellPressedRow - this.workerPos.row;
                        }
                    }
                } else if (cellPressedRow < this.workerPos.row) { //going up
                    for ( int i = cellPressedRow; i <= this.workerPos.row ;i++) {
                        if (!this.cells[i][this.workerPos.col].isFree() || this.cells[i][this.workerPos.col].hasBox()) {
                            isValidRoute = false;
                        }
                        else{
                            workerDir = "up";
                            distance= this.workerPos.row - cellPressedRow;
                        }
                    }

                }
            }
            else if (cellPressedRow == this.workerPos.row) {
                if (cellPressedColumn > this.workerPos.col) { //going right
                    for ( int i = this.workerPos.col; i <= cellPressedColumn; i++) {
                        if (!this.cells[this.workerPos.row][i].isFree() || this.cells[this.workerPos.row][i].hasBox()) {
                            isValidRoute = false;
                        }
                        else{
                            workerDir = "right";
                            distance = cellPressedColumn - this.workerPos.col;
                        }
                    }

                } else if (cellPressedColumn < this.workerPos.col) { //going left
                    for ( int i = cellPressedColumn; i <= this.workerPos.col; i++) {
                        if (!this.cells[this.workerPos.row][i].isFree() || this.cells[this.workerPos.row][i].hasBox()) {
                            isValidRoute = false;
                        }
                        else{
                            workerDir = "left";
                            distance = this.workerPos.col - cellPressedColumn;
                        }
                    }
    ...
    public void pull(String direction) {
        Position boxPos = workerPos.next(this.opposite(direction));
    ...
    private void undo() {
        if (this.actionH.size() < 1) return;

        // Pop off the last item in the stack
        ActionRecord action = this.actionH.pop();

        //
        if (action.isPush()) pull(this.opposite(action.direction()));
        if (action.isMove()) move(this.opposite(action.direction()));

        //
        this.undoH.push(action);
    }
     ...
     */
    private void redo() {
        if (this.undoH.size() < 1) return;

        // Pop off the last item in the stack
        ActionRecord action = this.undoH.pop();

        //
        if (action.isPush()) push(action.direction());
        if (action.isMove()) move(action.direction());

        this.actionH.push(action);
    }
